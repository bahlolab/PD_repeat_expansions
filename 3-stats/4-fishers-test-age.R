...
x = read.delim("../data/data_summary_counts/age/data_summary_all_cohort.txt")
  ...
  for(model in c(6:9)){
    for(cohort in cohorts) {
      ...
      for(locus in loci) {
        ...
        x1 = x[x$cohort %in% cohort & x$locus %in% locus & x$age_group %in% age_group,]
        ...
        if(nrow(x1) == 0){} else{
          ...
          mx_f = fisher.test(mx)
          res = rbind(res, c(cohort, age_group ,locus, paste0(case, "-control") ,colnames(x1[model]), c(mx_f$estimate, mx_f$p.value, mx_f$conf.int[1], mx_f$conf.int[2] ), n1, n2, n3, n4 ))
        }}}}}
...
colnames(res) = c("cohort", "age_group", "locus", "comparison", "inheritance", "OR", "p", "CIL", "CIU", "n1", "n2", "n3", "n4")

res = res[!(res$comparison %in% "Case-control" & res$cohort %in% "LBD"),]
...
write.table(res, file="fishers_test_EUR_age_group.txt", sep = "\t", quote = F, row.names = F)
